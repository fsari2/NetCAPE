...
c234567
     ...
     >                  cape_0C, cape_10C, cape_20C,                 
     >                  cape0C_cin, cape10C_cin, cape20C_cin,       
     >                  zlcl, zlfc, zel , psource , tsource , qvsource )
      ...
      real, intent(out) :: cape_0C, cape_10C, cape_20C
      real, intent(out) :: cape0C_cin, cape10C_cin, cape20C_cin
...
!
! NEW BY SLT
!           cape_0C - CAPE from LFC up to 0C isotherm (J/kg) (real)
!           cape_10C - CAPE from LFC up to -10C isotherm (J/kg) (real)
!           cape_20C - CAPE from LFC up to -20C isotherm (J/kg) (real)
!           cape0C_cin - CAPE_0C minus CIN (J/kg) (real)
!           cape10C_cin - CAPE_10C minus CIN (J/kg) (real)
!           cape20C_cin - CAPE_20C minus CIN (J/kg) (real)
!-----------------------------------------------------------------------
!  User options:

c234567
c     real, parameter :: pinc = 100.0   ! Pressure increment (Pa)
...
!-----------------------------------------------------------------------
      ...
      real, dimension(nk) :: p,t,td,pi,q,th,thv,z,pt,pb,pc,pn,ptv,ptd
      real, dimension(nk) :: pqv,pql
...
!New SLT; Considers water loading
      if(pt(k).gt.263.15) Print*,'pt(k),pql(k)= ',pt(k),pql(k)
      if(pt(k).gt.263.15) b2=b2-g*pql(k)
...
! Below added by SLT
!Note pt is parcel temperature in Kelvin
      if(pt(k).gt.273.15)cape_0C= cape_0C + max(0.0,parea)
      if(pt(k).gt.263.15)cape_10C= cape_10C + max(0.0,parea)
      if(pt(k).gt.253.15)cape_20C= cape_20C + max(0.0,parea)

      if(debug_level.ge.200)then
        write(6,102) p2,b1,b2,cape,cin,cape_0C,cape_10C,cape_20C,cloud
!102     format(5(f13.4),2x,l1)
102     format(8(f13.4),2x,l1)
      endif
...
!Below added by SLT
      cape0C_cin= cape_0C-cin
      cape10C_cin= cape_10C-cin
      cape20C_cin= cape_20C-cin
